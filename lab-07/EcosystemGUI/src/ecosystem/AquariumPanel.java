...
/**
 * World panel for the Aquarium simulation.
 * Contiene el tanque, peces, pellets, suciedad, etc.
 */
public class AquariumPanel extends JPanel
    ...
    public AquariumPanel(int w, int h) {
        this.worldW = w;
        this.worldH = h;
    ...
    // HUD (lo usa AquariumSim)
    public JPanel buildHUD() {
            ...
            switch (f.type) {
                case MID: {
                    if (f.hunger >= f.eatThreshold) {
                        ...
                        if (target != null && dist(f.x, f.y, target.x, target.y) <
                                f.size * 0.5 + target.radius) {
                            eatPellet(f, target);
                        }
                    }
                    break;
                ...
                case ALGAE: {
                    if (f.hunger >= f.eatThreshold && algaeLevel > 2) {
                        boolean nearWall = f.x < tankBounds.x + 40
                                || f.x > tankBounds.x + tankBounds.width - 40
                                || f.y > tankFloorY() - 40;
    ...
    private void chooseAndChaseTarget(Fish f, double dt) {
        ...
        if (tankBounds.contains(mouse)) {
            ...
            if (dToMouse < CURSOR_ATTRACT_RADIUS) {
                ...
                double followSpeed = f.speed * (0.8 + 0.4 *
                        (1.0 - Math.min(1.0, dToMouse / CURSOR_ATTRACT_RADIUS)));
        ...
        if (f.hunger >= f.eatThreshold) {
            Pellet target = (f.type == FishType.BOTTOM)
                    ? nearestPellet(f.x, f.y, true)
                    : nearestPellet(f.x, f.y, false);
            if (target != null) {
    ...
    private void updateStatus() {
        statusLabel.setText(String.format(
                ...
                fish.size(), pellets.size(), poops.size(), corpses.size(), dirt, algaeLevel,
                (filterOn ? "ON" : "OFF"),
    ...
    protected void paintComponent(Graphics g) {
        ...
        for (Pellet p : pellets) {
            ...
            g2.fill(new Ellipse2D.Double(p.x - p.radius, p.y - p.radius,
        ...
        for (Poop poop : poops) {
            ...
            g2.fill(new Ellipse2D.Double(poop.x - poop.radius, poop.y - poop.radius,
                    poop.radius * 2, poop.radius * 2));
        }
    ...
    private void paintWater(Graphics2D g2) {
        ...
        if (dirt > 0) {
            ...
            g2.setColor(new Color(dirtTint.getRed(), dirtTint.getGreen(), dirtTint.getBlue(),
                    (int) (alpha * 255)));
            g2.fillRect(tankBounds.x, waterY, tankBounds.width, waterH);
        }
        if (algaeLevel > 0) {
            float alpha = (float) Math.min(0.6, algaeLevel / 120.0);
            g2.setColor(new Color(algaeTint.getRed(), algaeTint.getGreen(), algaeTint.getBlue(),
                    (int) (alpha * 255)));
            g2.fillRect(tankBounds.x, waterY, tankBounds.width, waterH);
        }
    }
    ...
    private void paintFish(Graphics2D g2, Fish f) {
        ...
        tail.addPoint((int) (-bodyLen * 0.5 - tailLen),
                (int) (-bodyHt * (f.species.tailWide ? 0.5 : 0.35)));
        tail.addPoint((int) (-bodyLen * 0.5 - tailLen),
                (int) (bodyHt * (f.species.tailWide ? 0.5 : 0.35)));
        ...
        g2.fill(new Ellipse2D.Double(-bodyLen * 0.1, -bodyHt * 0.6,
                bodyHt * 0.45, bodyHt * 0.25)); // dorsal
        g2.fill(new Ellipse2D.Double(0, bodyHt * 0.2,
                bodyHt * 0.5, bodyHt * 0.25));  // ventral
        ...
        if (f.species.pattern == SpeciesPattern.STRIPES) {
            ...
            for (int i = -1; i <= 1; i++) {
                ...
                g2.fill(new Ellipse2D.Double(cx - bodyHt * 0.12,
                        -bodyHt * 0.12, bodyHt * 0.24, bodyHt * 0.24));
            }
        }

        // Eye
        g2.setColor(Color.WHITE);
        g2.fill(new Ellipse2D.Double(bodyLen * 0.22, -bodyHt * 0.2,
                bodyHt * 0.25, bodyHt * 0.25));
        g2.setColor(Color.BLACK);
        g2.fill(new Ellipse2D.Double(bodyLen * 0.27, -bodyHt * 0.15,
                bodyHt * 0.12, bodyHt * 0.12));
    ...
    private Fish makeMidWaterFish(double size) {
        return new Fish(FishType.MID, size, tankBounds, rng);
    }

    private Fish makeBottomFeeder(double size) {
        return new Fish(FishType.BOTTOM, size, tankBounds, rng);
    }

    private Fish makeAlgaeEater(double size) {
        return new Fish(FishType.ALGAE, size, tankBounds, rng);
    }
